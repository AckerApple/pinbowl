...
{ forceElement }) {
    ...
    if (isSubject) {
        const callback = (templateNewValue) => {
            console.log('ownerTag', ownerTag.insertBefore);
    ...
    if (before.id === '__tagVar0') {
        console.log('here removing - 0', result, ownerTag.tagSupport.templater?.wrapper.original);
    }
...
ownerTag, options) {
    ...
    if (result.tag) {
        ...
        delete result.tag;
        if (lastFirstChild.id === '__tagVar0') {
            console.log('here removing - 1', value, ownerTag.getTemplate());
        }
    ...
    if (before.id === '__tagVar0') {
        console.log('here removing - 2', result);
    }
...
{ index, counts, forceElement, }) {
    ...
    if (index !== undefined) {
        ...
        tag.buildBeforeElement(lastFirstChild, { counts, forceElement });
    ...
    tag.buildBeforeElement(before, { counts, forceElement });
